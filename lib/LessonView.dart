...
class LessonView extends StatefulWidget {
  ...
  LessonView(this.lesson, {required this.onChanged, required this.allTargetsVisible}) : this.state = lesson.data;
...
class _LessonViewState extends State<LessonView> {
  ...
  void onAudioButton(Vokabel vokabel) async {
    ...
    await audioPlayer.playFromPath(
      await DiContainer.resolve<LessonService>().unitLocalDirectory + vokabel.mp3,
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: new BoxDecoration(
            color: idx == this.widget.lesson.data.selected ? Color.fromRGBO(220, 220, 220, 1.0) : Colors.white),
        child: Row(
            ...
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                vokabel.target_sentence != "" && showTarget
                    ? Text(vokabel.target_sentence, style: TextStyle(fontSize: 18.0))
                    : empty,
                showTarget
                    ? SizedBox(
                        height: 10,
                ...
                vokabel.source_sentence != "" && showTarget
                    ? Text(vokabel.source_sentence, style: TextStyle(fontSize: 18.0))
                    : empty,
                showTarget
                    ? Row(
                        children: [
                ...
                FutureBuilder(
                  builder: (context, snapshot) {
                    return widget.lesson.data.selected == idx && snapshot.data != null
                        ? IconButton(
                    ...
                    child: Column(
                      children: <Widget>[
                        TextButton(
                            ...
                            style: ButtonStyle(backgroundColor: MaterialStateProperty.all(Color(0xee89fb98))),
    ...
    return Column(
      children: <Widget>[
        Container(
          ...
          child: Row(
            children: <Widget>[
              Text(widget.state.name == "" ? "unknown" : widget.state.name, style: body),
              Text("    ${widget.state.currentDone}/${widget.state.total - widget.state.done} (${widget.state.total})",
                  style: body),
            ],
          ),
        ),
